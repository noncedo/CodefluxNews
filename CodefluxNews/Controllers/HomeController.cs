    ...
    public class HomeController : Controller
    {
        ...
        public async Task<JsonResult> GetCategory(string Category, int pageIndex, int pageSize)
        {
            ...
            else
            {
                ...
                var imageCounter = _db.selectNewsArticles().Where(x=>x.CategoryName == Category).OrderByDescending(x => x.CreatedDate).Skip(pageIndex * pageSize).Take(pageSize).ToList();
